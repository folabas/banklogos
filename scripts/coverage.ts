...
for (const file of readdirSync(SNAPSHOT_DIR).filter((f) => f.endsWith('.json'))) {
  ...
  console.log(
    `${snapshot.scope} (${snapshot.regulator}, snapshot ${snapshot.fetchedAt}): ${covered.length}/${total} covered (${pct}%)`,
  );
  for (const { entry, entity } of covered) console.log(`  ✓ ${entry.legalName} -> ${entity.id}`);
  for (const { entry, entity } of nameOnly) {
    console.log(`  ~ ${entry.legalName} -> ${entity.id}: add "registryId": "${entry.registryId}" to its regulatorRef`);
  }
  for (const entity of stale)
    console.log(`  ! ${entity.id}: registryId ${entity.regulatorRef?.registryId} is no longer listed`);
